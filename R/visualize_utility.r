...
# plot.CUB <- function(reu13.df.obs, ret.b.Mat, phi.bin, estim.phi, n.use.iter=2000, rescale=F,
...
plot.likelihood.trace <- function(input)
{ 
  lay <- find.optimal.layout(length(input$res))
  par(oma=c(1,1,2,1), mgp=c(2,1,0), mar = c(3,4,2,1), mfrow=c(lay$x, lay$y))
  
  logTraces <- list()
  ll <- rep(0,length(input$res))
  for(i in 1:length(input$res))
  {
    logTraces[[i]] <- get.logL(input$res[[i]], input$data)
    ll[i] <- format(mean(as.numeric(logTraces[[i]][(length(logTraces[[i]])-config$use.n.iter):length(logTraces[[i]])])), digits = 10)
    plot(1:length(logTraces[[i]]), logTraces[[i]], 
         main=ll[i], xlab="Iteration", ylab="LogL")
    cat(paste(ll[i], "\n"))
...
plot.x.y <- function (x,y, main="x vs y", xlab="x", ylab="y")
{
  reg <- lm(log10(y)~log10(x))
  plot(log10(x),log10(y), xlab=paste("log10(",xlab,")", sep=""), ylab=paste("log10(",ylab,")", sep=""), main=main, 
       sub=paste("R^2=", format(summary(reg)$r.squared, digits = 3), sep=""))
  abline(reg, col="red", lwd=2) 
}

thinChain <- function(chain, gap.size=100)
{
  return(chain[seq(1, length(chain), gap.size)])
}

estimateEnvironmentProportions <- function(env1, env2, pred)
{
  optimize(f=function(p, env1, env2, pred)
           {
             env <- p*env1 + (1-p)*env2
             return(sqrt(sum((env-pred)^2)))
           }, interval=c(0,1), env1=env1, env2=env2, pred=pred, maximum=F)
}