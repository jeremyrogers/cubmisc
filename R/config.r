...
{
  cat("MCMC parameters:\n")
  cat(paste("Number of samples:", config$n.samples, "\n"))
...
config <- list(
  ...
  min.samples=200, # minimum samples each chain has to do, convergence criterium is ignored until min.iter is reached 
  max.samples=3000, # maximum samples for each chain. MCMC will be stoped if the chain reached max.iter iterations (convergence criterium is ignored)
  ...
  gf=0.1, ## growthfactor for convergence test window (n.samples + gf*currSamples)
  ...
  swapAt=0.5, # manhattan distance between two consequtive convergence test below which b matrix is swapped
