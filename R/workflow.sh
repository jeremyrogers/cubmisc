...
genome="../loganYeast/PrestonValues1.fasta"
...
prefix="loganYeast" # change name
suffix="withoutPhi"

folder="../results/nl/01-30/" # change folder
...
if [ "$cubmethod" = "cubappr" ]; then
	...
	nohup Rscript run_loganYeast.r -c $cubmethod -s "0.5 1 2 4" -f $genome -o $folder -n $foutname -i $pinit >> $logfile &
fi
